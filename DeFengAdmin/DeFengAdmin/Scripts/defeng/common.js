...
function InitResidentialDistrict(id, firstText, async) {
    //‘房配客’中默认已有两条记录，故不进行判断，直接请求加载记录
    if (id == "#matchResidentialDistrictSelect") {
        if (!($("" + id + " option").length == 2 && ($("" + id + " option").eq(1).text() == "五月花花园"))
            && $("" + id + " option").length != 0) {
            return;
        }
    } else {
        //如果已加载记录了，不再重新加载
        if (CheckSelectIsLoaded(id)) {
            return;
        }
    }
    $.ajax({
...
function InitHouseStatus(id, firstText, async) {
    if (id == '#houseStatusSearchSelect') {

    }
    else {//如果已加载记录了，不再重新加载
        if (CheckSelectIsLoaded(id)) {
            return;
        }
    }
...
/*
检测select中option是否已加载过
*/
function CheckSelectIsLoaded(id) {
    var loaded = true;
    var $options = $("" + id + " option");
    var len = $options.length;
    //如果已加载记录了，不再重新加载  有些select默认有一条option:' '或'全部'或'请选择',故条件如下
    loaded = ($("" + id).length == 0) || (!(len == 0) && !(len == 1 && ($options.eq(0).text() == " " || $options.eq(0).text() == "全部" || $options.eq(0).text() == "请选择")));
    //if (loaded) {
    //    console.log(id + '已加载。');
    //} else {
    //    console.log(id + '未加载。');
